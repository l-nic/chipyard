...
#include <time.h>
#include "encoding.h"
#define ARRAY_MIN (8*1024) /* 1/4 smallest cache */
#define ARRAY_MAX (16*1024*1024) /* 1/4 largest cache */
int x[ARRAY_MAX]; /* array going to stride through */
 
double get_seconds() { /* routine to read time in seconds */
  //__time64_t ltime;
  //_time64( &ltime );
  uint64_t ltime = rdcycle();
  return (double) ltime;
}
...
int main(int argc, char* argv[]) {
    ...
    for (csize=ARRAY_MIN; csize <= ARRAY_MAX; csize=csize*2) {
        ...
        for (stride=1; stride <= csize/2; stride=stride*2) {
            ...
            } while ((sec1 - sec0) < 20000.0); /* collect 20 seconds */
            sec = sec1 - sec0;
